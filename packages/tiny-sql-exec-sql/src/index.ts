import {
  ...
  import { TediousParams, getParams, isTediousParamLike } from "@australis/tiny-sql-params";
  ...
  export type ExecParams = TediousParams[] | ({}[]) | {};
  /**
   *
   */
  export default function execSql(connection: Connection){
    ...
    return async <T>(sqlTxt: string, args?: ExecParams) =>
      new Promise<Result<T>>(async (resolve, reject) => {
        ...
        const params: TediousParams[] = _getParams(args);
  ...
  function _getParams(args: TediousParams[] | ({}[]) | {}): TediousParams[] {
    if (Array.isArray(args) && args.length) {
      const value = args[0];
      if (isTediousParamLike(value)) return args as TediousParams[];
      return getParams(value);
    }
    if (typeof args === "object") return getParams(args);
    return [];
  }
  }